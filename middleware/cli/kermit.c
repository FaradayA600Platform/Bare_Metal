extern char fLib_getchar(void);
extern void fLib_putc(char Ch);

/* kermit.c come from armboot */
#define serial_getc fLib_getchar
#define serial_putc fLib_putc
...
/* entry function */
extern void set_kerm_bin_mode(unsigned long *addr);
extern int k_recv(void);
unsigned int load_serial_bin(unsigned int offset)
{
  set_kerm_bin_mode((unsigned int *)offset);
  k_recv();
  return offset;
}
