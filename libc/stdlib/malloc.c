...
//fLib_printf("sbrk increment = 0x%x\n", increment);
	/*
...
static uint8_t mem_malloc_pool[CONFIG_LIBC_MALLOC_SIZE] __attribute__((aligned(8)));

void mem_malloc_init(void)
{
//       fLib_printf("mem_malloc_init mem_malloc_start = 0x%x, mem_malloc_end = 0x%x\n", mem_malloc_start, mem_malloc_end);
	mem_malloc_start = (unsigned long)mem_malloc_pool;
	mem_malloc_end   = (unsigned long)mem_malloc_pool + CONFIG_LIBC_MALLOC_SIZE;
...
#else
static mchunkptr mremap_chunk(p, new_size) mchunkptr p;
size_t new_size;
#endif
{
	size_t page_mask = malloc_getpagesize - 1;
...
//void malloc_extend_top(INTERNAL_SIZE_T nb)
#else
static void malloc_extend_top(nb) INTERNAL_SIZE_T nb;
//void malloc_extend_top(nb) INTERNAL_SIZE_T nb;
#endif
{
...
//fLib_printf("malloc_extend_top DEFAULT_TOP_PAD = 0x%x nb = 0x%x, top_pad = 0x%x\n", DEFAULT_TOP_PAD, nb, top_pad);
...
//fLib_printf("mALLOc 1 bytes= 0x%x\n", bytes);
	if ((long)bytes < 0) return 0;

	nb = request2size(bytes);  /* padded request size; */
//fLib_printf("mALLOc 2 nb = 0x%x\n", nb);
...
//fLib_printf("mALLOc 5 chunksize(top)= 0x%x nb = 0x%x MINSIZE = 0x%x\n", chunksize(top), nb, MINSIZE);
...
#else
Void_t *cALLOc(n, elem_size) size_t n;
size_t elem_size;
#endif
{
	mchunkptr p;
...
#endif
{
fLib_printf("mALLOPt value = 0x%x\n", value);
	switch (param_number) {
