...
/*
 * Dante Su:
 * 1. Use putchar() when buffer is NULL.
 * 2. Add floating point support.
 * 3. Drop redundant functions for U-boot.
 */
#include <stdlib.h>
#include <stdarg.h>
#include <string.h>
...
#ifdef CONFIG_LIBC_PRINTF_STD

#include <linux/types.h>
#include <linux/compiler.h>

#ifndef CONFIG_SYS_VSNPRINTF
...
#ifdef CONFIG_LIBC_PRINTF_FPSUPP
		case 'f':
		case 'g':
			do {
				int i;
...
{
	return vsnprintf_internal(NULL, INT_MAX, fmt, args);
}
...
int printf(const char *fmt, ...)
{
	...
	va_start(args, fmt);
	ret = vsnprintf_internal(NULL, INT_MAX, fmt, args);
	va_end(args);
	return ret;
}

#endif /* CONFIG_LIBC_PRINTF_STD */
