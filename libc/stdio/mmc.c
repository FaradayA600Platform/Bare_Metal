...
DSTATUS disk_initialize(BYTE drv)
{
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		time_out = msTicks + 100;						/* Initialization timeout of 1000 msec */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);		/* Get trailing return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* The card can work at vdd range of 2.7-3.6V */
				while ((time_out > msTicks) && send_cmd(ACMD41, 1UL << 30));	/* Wait for leaving idle state (ACMD41 with HCS bit) */
				if ((time_out > msTicks) && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
			...
			while ((time_out > msTicks) && send_cmd(cmd, 0));			/* Wait for leaving idle state */
			if (!(time_out > msTicks) || send_cmd(CMD16, 512) != 0)	/* Set R/W block length to 512 */
				ty = 0;
		}
	}
	CardType = ty;
