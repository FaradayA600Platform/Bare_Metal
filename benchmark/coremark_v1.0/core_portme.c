...
extern unsigned int get_apb_clk(void);
...
timer_counter(void)
{
	...
	return (clock_t)fLib_Timer_Counter(1,0);
#elif defined(CONFIG_FTPWMTMR010)
	return (clock_t)fLib_Timer_Counter(1);
#endif
}

void init_fttmr(void)
{
//ee_printf("init_fttmr \n");
    if(!fLib_Timer_IOCtrl(IO_TIMER_SETTICK,1,1))
        return FALSE;

    fLib_Timer_AutoReloadValue(1,0xFFFFFFFF);
#if defined(CONFIG_FTTMR010)
    fLib_Timer_Counter(1,0xFFFFFFFF);
#elif defined(CONFIG_FTPWMTMR010)
    fLib_Timer_AutoReloadEnable(1);
#endif

    if(!fLib_Timer_Enable(1))
        return FALSE;

//    if(!fLib_Write_CR(1, 0, 0))
//        return FALSE;

#if 0
	/*
    FTTMR_REG32(FTTMRREG_T1CTRL) = 0;
    FTTMR_REG32(FTTMRREG_T1CMPB) = 0;
    FTTMR_REG32(FTTMRREG_T1CNTB) = 0xFFFFFFFF;
    FTTMR_REG32(FTTMRREG_T1CTRL) = FTTMRCTRL_AUTORELOAD | FTTMRCTRL_INTEN | FTTMRCTRL_START | FTTMRCTRL_UPDATE;
	*/
	fLib_Timer_Control(1, 0);
#if defined(CONFIG_FTPWMTMR010)
	fLib_Timer_CmpValue(1, 0);
#endif
	fLib_Timer_AutoReloadValue(1, 0xFFFFFFFF);
	fLib_Timer_Control(1, FTTMRCTRL_AUTORELOAD | FTTMRCTRL_INTEN | FTTMRCTRL_START | FTTMRCTRL_UPDATE);
#endif
...
#if defined (CONFIG_PLATFORM_FH36) || defined (CONFIG_PLATFORM_A369) || defined (CONFIG_PLATFORM_A500) || defined (CONFIG_PLATFORM_A600)
	#define NSECS_PER_SEC get_apb_clk()
#else
	#define NSECS_PER_SEC CLOCKS_PER_SEC
#endif
	#define EE_TIMER_TICKER_RATE 1000
	#define CORETIMETYPE clock_t 
//#if defined (BOARD_A320) || defined (BOARD_A360) || defined (CONFIG_PLATFORM_A369)
#if defined (CONFIG_PLATFORM_FH36) || defined (CONFIG_PLATFORM_A369) || defined (CONFIG_PLATFORM_A500) || defined (CONFIG_PLATFORM_A600)
	#define GETMYTIME(_t) (*_t=timer_counter())
#else
...
//#ifdef CONFIG_PLATFORM_A369
#if defined (CONFIG_PLATFORM_FH36) || defined (CONFIG_PLATFORM_A369) || defined (CONFIG_PLATFORM_A500) || defined (CONFIG_PLATFORM_A600)
//ee_printf("start_time_val = 0x%x stop_time_val = 0x%x\n", start_time_val, stop_time_val);
	CORE_TICKS elapsed=(CORE_TICKS)(MYTIMEDIFF(start_time_val, stop_time_val));
#else
