...
#include "flcd210.h"
#include "timer.h"
#include "flcd210_conf.h"

#define PAL_NUM	16 

extern UINT32 PANEL_WIDTH, PANEL_HEIGHT;
extern UINT32 LCD_PANEL_TYPE;
...
INT32 palette_check_seq()  //be used after palette_write_seq()
{ 
    UINT32 i, read;
    UINT32 *ptr;
...
#ifdef COMPLEX_OSD
    INT32 success=1;
    UINT32 pal[PAL_NUM], i;
    UINT32 *ptr;

    ptr = (UINT32 *) (base + 0x2040);
    for(i=0; i<PAL_NUM; i++)
        pal[i]=  *ptr++; 
...
void OSD_Pos( UINT32 which, INT32  HPos, INT32 VPos)
{
   ...
   switch (which) {
      case 1:
        temp = *(volatile UINT32 *) (LCD_IO_Base + 0x200C ) & ~0xffff;        
		*(volatile UINT32 *) (LCD_IO_Base + 0x200C ) = temp | (VPos << 8) | HPos;	
		break;
     case 2:
        temp = *(volatile UINT32 *) (LCD_IO_Base + 0x2018 ) & ~0xffff;         
		*(volatile UINT32 *) (LCD_IO_Base + 0x2018 ) = temp | (VPos << 8) | HPos;	
		break;
     case 3:
        temp = *(volatile UINT32 *) (LCD_IO_Base + 0x2024 ) & ~0xffff;         
		*(volatile UINT32 *) (LCD_IO_Base + 0x2024 ) = temp | (VPos << 8) | HPos;
		break;	
     case 4:
        temp = *(volatile UINT32 *) (LCD_IO_Base + 0x2030 ) & ~0xffff;         
		*(volatile UINT32 *) (LCD_IO_Base + 0x2030 ) = temp | (VPos << 8) | HPos;	
		break;
	}
}   

void OSD_Dim( UINT32 which, INT32 HDim, INT32 VDim)	//define the total font number of each row and the total row number
{
	UINT32 value;
	...
	{
	case 1:
		value = *(volatile UINT32 *) (LCD_IO_Base + 0x2010);
		...
		*(volatile UINT32 *) (LCD_IO_Base + 0x2010) = value;
		break;
	case 2:
		value = *(volatile UINT32 *) (LCD_IO_Base + 0x201C);
		value &= ~0x7f;	//clear the horizontal and vertical dimension of the OSD window
		value &= ~(0x7f << 8);
		value |= ((VDim << 8) | HDim );	//set horizontal and vertical dimension
		*(volatile UINT32 *) (LCD_IO_Base + 0x201C) = value;
		break;
	case 3:
		value = *(volatile UINT32 *) (LCD_IO_Base + 0x2028);
		value &= ~0x7f;	//clear the horizontal and vertical dimension of the OSD window
		value &= ~(0x7f << 8);
		value |= ((VDim << 8) | HDim );	//set horizontal and vertical dimension
		*(volatile UINT32 *) (LCD_IO_Base + 0x2028) = value;
		break;
	case 4:
		value = *(volatile UINT32 *) (LCD_IO_Base + 0x2034);
		value &= ~0x7f;	//clear the horizontal and vertical dimension of the OSD window
		value &= ~(0x7f << 8);
		value |= ((VDim << 8) | HDim );	//set horizontal and vertical dimension
		*(volatile UINT32 *) (LCD_IO_Base + 0x2034) = value;
		break;
	}
} 
...
UINT32 OSD_control(UINT32 hightlight,UINT32 bdType, UINT32 bdColor, UINT32 shType, UINT32 shColor )
{
   UINT32 value;
...
{
	UINT32 value;	//volatile is needed, or it is wrong 
...
{	
	...
	*(volatile UINT32 *) (LCD_IO_Base + 0x2004) &= 0xff0083ff;	   
	*(volatile UINT32 *) (LCD_IO_Base + 0x2004) |= ((TranType << 10) | (HighLightType << 13) | (BdColorSel << 16) | (ShColorSel << 20));	//bit 12-10 :set OSD font transparency, 0=0%, 1=25%, 2=50%, 3=75%, 4=100%
}

void OSD_Row_Col_Space( UINT32 RowSpace, UINT32 ColSpace)
{
	*(volatile UINT32 *) (LCD_IO_Base + 0x2008) &= 0xffffe0e0;
	
	if (RowSpace <= 15) {
		*(volatile UINT32 *) (LCD_IO_Base + 0x2008) |= 0x00000001;
		*(volatile UINT32 *) (LCD_IO_Base + 0x2008) |= (RowSpace << 1);
	} 
	else
		printf("Row space set error\n");
		
	if (ColSpace <= 15) {
		*(volatile UINT32 *) (LCD_IO_Base + 0x2008) |= 0x00000100;
		*(volatile UINT32 *) (LCD_IO_Base + 0x2008) |= (ColSpace << 9);
	} 
	else
...
void OSD_FontAttributeBase( UINT32 which, UINT16 fontIndex )
{
	...
	{
		case 1:
			*(volatile UINT32 *)(LCD_IO_Base + 0x2014) = fontIndex;	//indicating the start index of font attribute RAM for OSD window 1
			break;
		case 2:
			*(volatile UINT32 *)(LCD_IO_Base + 0x2020) = fontIndex;
			break;
		case 3:
			*(volatile UINT32 *)(LCD_IO_Base + 0x202C) = fontIndex;
			break;
		case 4:
			*(volatile UINT32 *)(LCD_IO_Base + 0x2038) = fontIndex;
			break;
	}
}
...
{
	*(volatile UINT32 *)(LCD_IO_Base + 0x203C) =  (UINT32) fontbase;	//indicating the start address of multi-color font attribute RAM
}


/*HZoomIn=0 ->HScal=0,1; HZoomIn=1 ->HScal=0,1,2,3   VZoomIn=0 ->VScal=0,1; VZoomIn=1 ->VScal=0,1,2,3 */
void OSD_Scal( INT32 HZoomIn, INT32 HScal, INT32 VZoomIn, INT32 VScal)	//define the horizontal and vertical scaling factor
{
	UINT32 value;

	value = *(volatile UINT32 *) (LCD_IO_Base + 0x2004);
	...
	*(volatile UINT32 *) (LCD_IO_Base + 0x2004) = value;
}

//we can use this function only when there are all single fonts, or all MCF fonts
void OSD_putc( INT8 c, INT32 position, UINT16 foreground, UINT16 background )	//map the OSC_Font[], ie. set font index in the font attribute RAM
{
 ...
 // 	if( *(volatile  UINT32 * )( LCD_IO_Base + 0x203C ) == NO_MCF_FONT || *( UINT32 * )( LCD_IO_Base + 0x203C ) == 0 )
  	{
		if( c >= '0' && c <= '9' )
			*(volatile UINT32 *) (LCD_IO_Base + 0x20000 + position *4 )  = ( (c -'0') << 8) | (foreground << 4) | background;  
		else if( c>= 'A' && c <= 'Z') 
			*(volatile UINT32 *) (LCD_IO_Base + 0x20000 + position *4 )  = ( ( c - 'A' +10) << 8) | (foreground << 4) | background;       
		else if( c == ' ' )
			*(volatile UINT32 *) (LCD_IO_Base + 0x20000 + position *4 )  = ( ( 'Z' - 'A' +10 + 1) << 8) | (foreground << 4) | background; 
		else if( c == '=')
			*(volatile UINT32 *) (LCD_IO_Base + 0x20000 + position *4 )  = ( ( 'Z' - 'A' +10 + 2) << 8) | (foreground << 4) | background;  
		else if( c == ',')
			*(volatile UINT32 *) (LCD_IO_Base + 0x20000 + position *4 )  = ( ( 'Z' - 'A' +10 + 3) << 8) | (foreground << 4) | background;
/*	}else{
...
void OSD_On( UINT32 which )  //OSD Enable/disable
{
	if (which > 0xF) {
		printf("Wrong OSD window id\n");
		return;
	}
	*(volatile UINT32 *) LCD_IO_Base |= 0x10;	//OSD windows on

	*(volatile UINT32 *)(LCD_IO_Base + 0x2000) &= 0xfffffff0;
	*(volatile UINT32 *)(LCD_IO_Base + 0x2000) |= which;	//enable the sub-window
}

void OSD_Off()  //OSD disable
{
	*(volatile UINT32 *) LCD_IO_Base &= 0xffffffef;
	*(volatile UINT32 *)(LCD_IO_Base + 0x2000) &= 0x0;
}

void OSD_Test_Complex(INT8 *str)
{
	INT32 i,j,x,y, k, ratio;
	INT8 msg[64];
	UINT8 r, g, b;
	BOOL mcf_used = TRUE;		//use MCF font???
...
//	SetBGRSW( BGR );
	...
	r=0xff;	g=0;	b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2040) = rgb2ycbcr( &r, &g, &b );  // Palette 0 = Red
	r=0;	g=0xff; b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2044) = rgb2ycbcr( &r, &g, &b );  // Palette 1 = Green	
	r=0;	g=0;	b=0xff;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2048) = rgb2ycbcr( &r, &g, &b );  // Palette 2 = Blue		
	r=36;	g=16;	b=16;
	*(volatile UINT32 *) (LCD_IO_Base + 0x204C) = rgb2ycbcr( &r, &g, &b );  // Palette 3 = similar Black	
	r=0xff;	g=0xff; b=0xff;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2050) = rgb2ycbcr( &r, &g, &b );  // Palette 4 = White	
	r=0xaa;	g=0;	b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2054) = rgb2ycbcr( &r, &g, &b );  // Palette 5 = Red
	r=0;	g=0xaa; b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2058) = rgb2ycbcr( &r, &g, &b );  // Palette 6 = Green
	r=0;	g=0;	b=0xaa;
	*(volatile UINT32 *) (LCD_IO_Base + 0x205C) = rgb2ycbcr( &r, &g, &b );  // Palette 7 = Blue
	r=0x45;	g=0x45;	b=0x45;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2060) = rgb2ycbcr( &r, &g, &b );  // Palette 8 = Black
	r=0x90;	g=0x90; b=0x90;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2064) = rgb2ycbcr( &r, &g, &b );  // Palette 9 = White
	r=0x55;	g=0;	b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2068) = rgb2ycbcr( &r, &g, &b );  // Palette 10 = Red
	r=0;	g=0x55;	b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x206C) = rgb2ycbcr( &r, &g, &b );  // Palette 11 = Green
	r=0;	g=0;	b=0x55;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2070) = rgb2ycbcr( &r, &g, &b );  // Palette 12 = Blue
	r=0x20;	g=0x50;	b=0x70;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2074) = rgb2ycbcr( &r, &g, &b );  // Palette 13 = Black
	r=0xff;	g=0xf; b=0xf0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2078) = rgb2ycbcr( &r, &g, &b );  // Palette 14 = White
	r=0x11;	g=0;	b=0;
	*(volatile UINT32 *) (LCD_IO_Base + 0x207C) = rgb2ycbcr( &r, &g, &b );  // Palette 15 = Red

	if(mcf_used == TRUE){
		OSD_MCFBase( 0 );
		for(i=0; i< 18* 10* 3 + 18* 26* 3 + 18* 1* 3; i++)
		*(volatile UINT32 *) (LCD_IO_Base + 0x10000 + i*4 ) = OSD_MCFont[i];
	}else{
		OSD_MCFBase( NO_MCF_FONT );
		for(i=0; i< 18* 10 + 18* 26 + 18* 3  ; i++)   // 10 digit & 26 alphabet & 3 symbol
			*(volatile UINT32 *) (LCD_IO_Base + 0x10000 + i*4 )  = OSD_Font[i];	// i*4 -> 1 word = 4 bytes
	}
...
*/
	...
	for(i=0; i< 320  ; i++)
		*(volatile UINT32 *) (LCD_IO_Base + 0x20000 + i*4 )  = OSD_Attribute[i];      
...
void OSD_On_Simple( UINT8 on)  //OSD Enable/disable
{
    *(volatile UINT32 *) (LCD_IO_Base) &= 0xffffffef;	//OSD windows off
    *(volatile UINT32 *) (LCD_IO_Base) |= on << 4;	//OSD windows on
}


void OSD_Pos_Simple( INT32  HPos, INT32 VPos )
{
   *(volatile UINT32 *) (LCD_IO_Base + 0x2004) = (HPos << 12) | VPos;
   *(volatile UINT32 *) (LCD_IO_Base + 0x2000) |= 1 << 20;	//update OSD parameters
}   


void OSD_Dim_Simple( INT32 HDim, INT32 VDim )	//define the total font number of each row and the total row number
{
    volatile UINT32 value;

    value = *(volatile UINT32 *) (LCD_IO_Base + 0x2000);
    ...
    *(volatile UINT32 *) (LCD_IO_Base + 0x2000) = value;
	*(volatile UINT32 *) (LCD_IO_Base + 0x2000) |= 1 << 20;	//update OSD parameters
} 
...
{
   *(volatile UINT32 *) (LCD_IO_Base + 0x200C) &= 0xfffffffc;	//set OSD background transparency = 25%
   *(volatile UINT32 *) (LCD_IO_Base + 0x200C) |= level;	//bit 5-4 :set OSD background transparency, 00=25%, 01=50%, 10=75%, 11=100%
}


void OSD_fg_color_Simple( INT32 pal0, INT32 pal1, INT32 pal2, INT32 pal3)	//OSD foreground color control
{
    *(volatile UINT32 *) (LCD_IO_Base + 0x2008) = (pal0) | (pal1 <<8) | (pal2 << 16) | (pal3<< 24);	//palette entry 0~3
}
...
void OSD_bg_color_Simple( INT32 pal1, INT32 pal2, INT32 pal3)
{
    ...
    value = *(volatile UINT32 *) (LCD_IO_Base + 0x200C);
    value &= 0x000000ff;
    value |= (pal1 <<8) | (pal2 << 16) | (pal3 << 24);
    *(volatile UINT32 *) (LCD_IO_Base + 0x200C) = value;
}


void OSD_Scal_Simple( INT32 HScal, INT32 VScal)	//define the horizontal and vertical up-scaling factor
{
    volatile UINT32 value;
...
void OSD_putc_Simple( INT8 c, INT32 position)	//map the OSC_Font[]
{
    if( c >= '0' && c <= '9' )
        *(volatile UINT32 *) (LCD_IO_Base + 0xc000 + position *4 )  = ( (c -'0') << 4);  
    else if( c>= 'A' && c <= 'Z')  
        *(volatile UINT32 *) (LCD_IO_Base + 0xc000 + position *4 )  = ( ( c - 'A' +10) << 4); 

    if( c == ' ' )
        *(UINT32 *) (LCD_IO_Base + 0xc000 + position *4 )  = ( ( 'Z' - 'A' +10 + 1) << 4); 

    if( c == '=')
        *(volatile UINT32 *) (LCD_IO_Base + 0xc000 + position *4 )  = ( ( 'Z' - 'A' +10 + 2) << 4);  

    if( c == ',')
        *(volatile UINT32 *) (LCD_IO_Base + 0xc000 + position *4 )  = ( ( 'Z' - 'A' +10 + 3) << 4);       
}
...
//  UINT8 r, g, b;
    ...
    for(i=0; i< font_col*10 + font_col* 26 + 3* font_col; i++)   // 10 digit & 26 alphabet & 3 symbol
        *(volatile UINT32 *) (LCD_IO_Base + 0x8000 +i*4 ) = OSD_Font_Simple[i];	// i*4 -> 1 word = 4 bytes

    for(i=0; i< 100  ; i++)
        *(volatile UINT32 *) (LCD_IO_Base + 0xc000 +i*4 ) = OSD_Attribute_Simple[i];        
