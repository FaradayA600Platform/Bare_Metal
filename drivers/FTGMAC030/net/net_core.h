...
#define debug(fmt, args...) debug_cond(1, fmt, ##args)
 ...
 */
typedef void rxhand_f(uint8_t *pkt, unsigned dport,
...
typedef void rxhand_icmp_f(unsigned type, unsigned code, unsigned dport,
		IPaddr_t sip, unsigned sport, uint8_t *pkt, unsigned len);
...
extern uint32_t		NetBootFileXferSize;	/* size of bootfile in bytes */
extern uint8_t		NetOurEther[6];		/* Our ethernet address */
extern uint8_t		NetServerEther[6];	/* Boot server enet address */
extern IPaddr_t		NetOurIP;	/* Our    IP addr (0 = unknown) */
extern IPaddr_t		NetServerIP;	/* Server IP addr (0 = unknown) */
extern uint8_t		*NetTxPacket;		/* THE transmit packet */
extern uint8_t		*NetRxPackets[PKTBUFSRX]; /* Receive packets */
extern uint8_t		*NetRxPacket;		/* Current receive packet */
extern int		NetRxPacketLen;		/* Current rx packet length */
extern unsigned		NetIPID;		/* IP ID (counting) */
extern uint8_t		NetBcastAddr[6];	/* Ethernet boardcast address */
extern uint8_t		NetEtherNullAddr[6];
...
extern int NetSetEther(uint8_t *, uint8_t *, uint);
extern int net_update_ether(struct ethernet_hdr *et, uint8_t *addr, uint prot);

/* Set IP header */
extern void net_set_ip_header(uint8_t *pkt, IPaddr_t dest, IPaddr_t source);
extern void net_set_udp_header(uint8_t *pkt, IPaddr_t dest, int dport,
				int sport, int len);

/* Checksum */
extern int	NetCksumOk(uint8_t *, int);	/* Return true if cksum OK */
extern uint	NetCksum(uint8_t *, int);		/* Calculate the checksum */
...
extern void	NetSetTimeout(uint32_t, thand_f *);/* Set timeout handler */
...
/* Transmit a packet */
static inline void NetSendPacket(uint8_t *pkt, int len)
{
	if (len < 64)
		len = 64;
 ...
 */
extern int NetSendUDPPacket(uint8_t *ether, IPaddr_t dest, int dport,
			int sport, int payload_len);

/* Processes a received packet */
extern void NetReceive(uint8_t *, int);
...
/* return uint32_t *in network byteorder* */
static inline uint32_t NetReadLong(uint32_t *from)
{
	uint32_t l;
...
/* copy uint32_t */
static inline void NetCopyLong(uint32_t *to, uint32_t *from)
{
